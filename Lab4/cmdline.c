...
void CommandSettings(uint_fast8_t ui8Argc, char *g_ppcArgv[]) {
	if (ui8Argc < 2) {
		OutCRLF();
		UART_OutString("Insufficient arguments for command settings");
		return;    
	}
	
	if (strcmp(g_ppcArgv[1], "fir")==0) {
		if (ui8Argc < 3) {
			OutCRLF();
			UART_OutString("Insufficient arguments for command settings fir");
			return;    
		}
		if (strcmp(g_ppcArgv[2], "on")!=0 && strcmp(g_ppcArgv[2], "off")!=0) {
			OutCRLF();
			UART_OutString("Invalid arguments for command settings fir");
			return;
		}
		OS_Wait(&Settings);
		ADCSettings.fir = (strcmp(g_ppcArgv[2], "on")==0) ? on : off;
		OS_Signal(&Settings);
	}
	
	if (strcmp(g_ppcArgv[1], "trigger")==0) {
		if (ui8Argc < 3) {
			OutCRLF();
			UART_OutString("Insufficient arguments for command settings trigger");
			return;    
		}
		if (strcmp(g_ppcArgv[2], "switch")!=0 && strcmp(g_ppcArgv[2], "threshold")!=0 && strcmp(g_ppcArgv[2], "continuous")!=0) {
			OutCRLF();
			UART_OutString("Invalid arguments for command settings trigger");
			return;
		}
		OS_Wait(&Settings);
	...
	if (strcmp(g_ppcArgv[1], "print")==0) {
		OutCRLF();
		switch(ADCSettings.trigger) {
			case switch1:
				UART_OutString("\r\nADC Trigger: switch1");
				break;
			case threshold:
				...
				break;
			case continuous:
				UART_OutString("\r\nADC Trigger: continuous");
				break;
		}
		
		switch(ADCSettings.fir) {
			case on:
				UART_OutString("\r\nFIR: on");
				break;
			case off:
